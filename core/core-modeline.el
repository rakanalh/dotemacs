...
(defun +doom-modeline|update-env ()
  (when +doom-modeline-env-command
    (let* ((default-directory (get-root))
...
buffer where knowing the current project directory is important."
  (let ((face (if (active) 'doom-modeline-buffer-path)))
    (concat (all-the-icons-octicon
             ...
             :v-adjust -0.05
             :height 1.25)
            (propertize (concat " " (abbreviate-file-name (get-root)))
                        'face face))))
...
(def-modeline-segment! pyenv
  ...
  (when (and (eq 'python-mode major-mode)
    ...
    (let ((name (pyenv-mode-version)))
      (concat +doom-modeline-vspc
	      "  "
	      (all-the-icons-alltheicon "python" :face 'all-the-icons-dblue :height 1.0 :v-adjust 0.05)
              " "
              (propertize name 'help-echo "Virtual environment (via pyenv)")
              "  "
              +doom-modeline-vspc))))
...
(def-modeline! main
  ...
  (buffer-encoding vcs major-mode pyenv flycheck))
